...
def detect_circle_by_hough(frame):
    ...
    gray = cv2.GaussianBlur(gray, (5, 5), 1.2,1.2);
    ...
    circles = cv2.HoughCircles(gray, cv2.HOUGH_GRADIENT, 1, 100, param1=6, param2=2, minRadius=min_radus,
                               maxRadius=height)
    """
...
#input_shape=(275,275)
def evalimage(img=None,output_dir='',session=None,input_shape=(175,175),):
    sess=session
    ...
    h_rate=1.0*high/input_shape[0]
    w_rate = 1.0 * width / input_shape[1]
    # print('h_rate',h_rate)
    # print('w_rate', w_rate)
    ...
    batch = pre_process(frame.unsqueeze(0))
    ...
    if len(circle_) > 0:
        cv2.circle(original_img, (int(circle_[0]*h_rate), int(circle_[1]*w_rate)), int(circle_[2]*h_rate), (0, 255, 0), 4)
    return original_img
'''
    ...
    img_numpy=cv2.resize(img_numpy,(int(input_shape[0]*h_rate),int(input_shape[1]*w_rate)))
    # print((int(input_shape[0]*h_rate),int(input_shape[1]*w_rate)))
    # cv2.imshow('',img_numpy)
    # cv2.waitKey(100)
    circle_ = detect_circle_by_hough(img_numpy)
    # print('circle :',circle_)
    # print('circle :', circle_[0]*h_rate,circle_[1]*w_rate)
    #
    # if len(circle_) > 0:
    #     cv2.circle(img_numpy, (circle_[0], circle_[1]), circle_[2], (0, 255, 0))
    if len(circle_) > 0:
        cv2.circle(original_img, (circle_[0], circle_[1]), circle_[2], (0, 255, 0), 4)
        # cv2.imshow('',img)
        # cv2.waitKey(100)
...
def circledetection(input_shape=(640, 640), image_path='./image',
                    output_dir='./outputs', session1=None, session2=None):
    ...
    ort_inputs = {session1.get_inputs()[0].name: img[None, :, :, :]}
    output = session1.run(None, ort_inputs)
    ...
    if dets is not None:
        ...
        if final_scores[index] < 0.97:
            # 调用霍夫变换算法
            #evalimage(imgname=path + "/" + imgname, output_dir="./to_test_out", model_path=model_path, session=sess)
            #print('不完整检测')
            origin_img=evalimage(origin_img,output_dir=output_dir,session=session2)

        else:
            final_box = final_boxes[index]
            ...
            if countNum > 0:
                ...
                cv2.circle(origin_img, (center_x, center_y), r, (0, 255, 0), 4)

    output_path = os.path.join(output_dir, os.path.basename(image_path))
    cv2.imwrite(output_path, origin_img)


def loadmodel(model_path='./yolox_s_512.onnx'):
    return onnxruntime.InferenceSession(model_path)

if __name__ == '__main__':
    path = './image_2'
    model_path = './yolox_s_512.onnx'
    yolact_model_path = "yolact_resnet50_275x275.onnx"
    session2 = rt.InferenceSession(yolact_model_path)
    starttime=time.time()
    session1 = loadmodel(model_path=model_path)
    ...
    for imgname in os.listdir(path):
        ...
        starttime = time.time()
        for i in range(2):
            circledetection(input_shape=(512, 512), image_path=path + "/" + imgname,
                            output_dir='./outputs_3', session1=session1,session2=session2)
        print('infer time', (time.time() - starttime)/2.0)